	...
	loop {
		...
		terminal.draw(|f| {
			...
			let chunks = Layout::default()
				...
				.split(size);
			let titles =
				labels.iter().cloned().map(Spans::from).collect();
		...
		match rx.recv() {
			...
			{
				let item = contents[current_tab_index]
					.remove(current_select_item_index);
				...
				if !contents[current_tab_index].is_empty() =>
			{
				current_select_item_index =
					(current_select_item_index
						+ contents[current_tab_index].len()
						- 1) % contents[current_tab_index].len();
			}
			Ok(KeyCode::Backspace)
				if !contents[current_tab_index].is_empty() =>
			{
				contents[current_tab_index]
					.remove(current_select_item_index);
			...
			Ok(KeyCode::Tab) => {
				current_tab_index =
					(current_tab_index + 1) % labels.len();
			}
			Ok(KeyCode::BackTab) => {
